...
export function ButtonLink({ href, children, variant = "dark", className = "" }: ButtonLinkProps) {
  ...
  if (href.startsWith("http") || href.startsWith("mailto:") || href.endsWith(".pdf")) {
    return (
      <a href={href} className={allClasses} target={href.startsWith("http") || href.endsWith(".pdf") ? "_blank" : undefined} rel={href.startsWith("http") || href.endsWith(".pdf") ? "noreferrer" : undefined}>
        {children}
      </a>
    );
  }

  return <Link href={href} className={allClasses}>{children}</Link>;
}
