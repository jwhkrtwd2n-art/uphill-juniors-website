    ...
    <main className="px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-2">
        <div className="rounded-[2rem] bg-slate-950 p-8 text-white shadow-xl">
          ...
          <div className="mt-6"><ButtonLink href={`mailto:${CLUB_EMAIL}?subject=Volunteering%20with%20Uphill%20Juniors%20FC`} variant="sky">Ask about volunteering</ButtonLink></div>
        </div>
        <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <div className="mt-6"><ButtonLink href={`mailto:${CLUB_EMAIL}?subject=Sponsorship%20Enquiry%20-%20Uphill%20Juniors%20FC`}>Send sponsor enquiry</ButtonLink></div>
        </div>
      </div>
      <div className="mx-auto mt-8 max-w-6xl rounded-3xl bg-sky-50 p-6 text-center">
        <p className="font-black text-slate-950">Email: <a className="text-sky-700 underline" href={`mailto:${CLUB_EMAIL}`}>{CLUB_EMAIL}</a></p>
      </div>
    </main>
  );
}
